...
##拼接查询query
STRING2='{   "query": {     "match": {       "comment": {         "query": "'${SEARCH_STRING}'",         "minimum_should_match": "75%"       }     }   },  "_source": [ "command", "comment" ] }'
...
do 
     ...
     [ $FLAG == '"command"' ] && echo "${line}" >> /tmp/tmp_command.txt || echo "${line}" >> /tmp/tmp_comment.txt
...
if [ "$LINE_NUM" -gt 0 -a $LINE_NUM -le $SUMARY  ] 2>/dev/null ;then 
    ...
    echo -e "\033[42;37m ${COMMAND} \033[0m"
    read -p "执行请输Y/y:" EXECUTE 
    [ "$EXECUTE" == "Y" -o "$EXECUTE" == "y" -o "$EXECUTE" == "yes" -o "$EXECUTE" == "YES" -o "$EXECUTE" == "Yes" ]  && eval ${COMMAND} 
else 
    echo '重新输入！！' 
fi
