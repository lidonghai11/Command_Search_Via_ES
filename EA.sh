...
#可打开查看查询的结果
#echo $FILTER
#curl -u elastic:1234567\# -XGET "http://8.210.23.127:9200/commandslist/_search?pretty" -H 'Content-Type: application/json' -d"$FILTER"
> /tmp/same.txt #清空上次的查询结果
#查询相似命令
curl -u elastic:1234567\# -XGET "http://$IP:$PORT/commandslist/_search?pretty" -H 'Content-Type: application/json' -d"$FILTER"  2>/dev/null  | egrep -w '"command"' | awk -F '"command" : ' '{print $2}' > /tmp/same.txt
...
if [ $LINES -eq 0 ];then
  ...
  curl  -u elastic:1234567\#  -XPOST "http://$IP:$PORT/commandslist/_doc" -H 'Content-Type: application/json' -d"$QUERY"  #> /dev/null 2>&1
  #echo 'kc'
  exit 0
else
  while read line
